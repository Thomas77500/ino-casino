...
create policy if not exists "casino_progress_select_all" on public.casino_progress
  for select to authenticated using (true);

create policy if not exists "casino_progress_insert_own" on public.casino_progress
  for insert with check (auth.uid() = user_id);

create policy if not exists "casino_progress_update_own" on public.casino_progress
